...
app = FastAPI(title="CloudVulnTracker (Heuristic Only)")
...
def heuristic_classify(text: str):
    """Analiza tekstu: kto jest odpowiedzialny (GCP / Client / Both / Unknown)."""
    t = text.lower()
...
async def scrape_bulletins(url: str):
    ...
    for header in soup.find_all(["h2", "h3"], id=re.compile(r"^gcp-\d{4}-\d{3}")):
        ...
        if not cves:
            continue  # pomijamy wpisy bez CVE

        # Daty publikacji i aktualizacji
        published = None
        updated = None
        # znajdź wszystkie daty w formacie YYYY-MM-DD
...
async def get_bulletins(product: str = Query(None, description="np. gke, compute, sql, mesh (opcjonalnie)")):
    """Pobiera biuletyny bezpieczeństwa GCP i klasyfikuje odpowiedzialność (heurystycznie)."""
    if product:
        if product not in DOC_URLS:
            raise HTTPException(status_code=400, detail=f"Nieznany produkt: {product}")
...
@app.get("/stats")
async def get_stats():
    """Zwraca statystyki klasyfikacji biuletynów."""
    tasks = [scrape_bulletins(url) for url in DOC_URLS.values()]
    results_all = await asyncio.gather(*tasks)
    merged = [item for sublist in results_all for item in sublist]
